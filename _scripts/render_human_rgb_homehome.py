...
odn = f'/HOME/HOME/data/panic3d/_data/lustrous/renders/{name}' if not DEBUG else f'/dev/shm/_mgl_test/{name}'
# pdb.set_trace()
# render_dtype = 'rgb_100view_2' # dlw changed 20231104
render_dtype = 'rgb_100view_dlw' # dlw changed 20231104
...
boxwarp = 2.1
# frust_near,frust_far = 2.5, 4.5# 0.5, 1.5
...
for i in range(n_generations):
    ...
    azimuths.append(azimuth_mu + 3.6*i - 180.0)
    ...
    def __init__(self, program=None, loc_origin=None, **kwargs):
        ...
        self.program = mglw.resources.programs.load(
            mglw.meta.ProgramDescription(
                ...
                path=Path("/HOME/HOME/data/panic3d/gameday-3d-human-reconstruction-panic3d_renderer-/_cells/shaders/rutileE.glsl").resolve(),
            )
        )
