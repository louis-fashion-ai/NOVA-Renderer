...
import pygltflib
import sys
sys.path.append('/HOME/data/panic3d/gameday-3d-human-reconstruction-panic3d_renderer-')
sys.path.append('/HOME/data/panic3d')
...
# if bn[0] in ['0', '1','2','3','4']:
#     print(f'JOB {bn} ALREADY')
#     exit(0)
# franch = 'debug'
# bn = fnstrip(ifn)
# name,franch,bn = ap.parse_args().name.split(',')
...
odn = f'/HOME/data/panic3d/_data/lustrous/renders/{name}' if not DEBUG else f'/dev/shm/_mgl_test/{name}'

render_dtype = 'rgb_100view_2'
#render_dtype = 'rgb_48view'
...
frust_near,frust_far = 2.5, 4.5
# frust_near,frust_far = 1.0, 3.0
assert name in {'daredemoE', 'rutileE', 'human_rutileE'}
...
#全身
elevation_mu, elevation_sig = 3.8, 20
azimuth_mu, azimuth_sig = 0.0, 100000  # basically uniform
distance_mu, distance_sig = 3.5, 0.0
fov_mu, fov_sig = 30.0, 0.0

#半身
# n_generations = 100 if not DEBUG else 16
# # elevation_mu, elevation_sig = 3.8, 20
# elevation_mu, elevation_sig = 0.0, 20
# # azimuth_mu, azimuth_sig = 0.0, 100000  # basically uniform
# azimuth_mu, azimuth_sig = 180, 100000
# # distance_mu, distance_sig = 3.5, 0.0
# # distance_mu, distance_sig = 2.0, 0.0
# distance_mu, distance_sig = 1.0, 0.0
# fov_mu, fov_sig = 30.0, 0.0  # full-fov, only zero std supported atm
...
fovs = []
for i in range(n_generations):
    elevations.append(elevation_mu)
    azimuths.append(azimuth_mu + 3.6*i - 180.0) #azimuths.append(azimuth_mu + 7.5*i - 180.0)
    distances.append(distance_mu)
    fovs.append(fov_mu)


# other render params
offset_head = np.asarray([0, -0.6, 0])
    ...
    def __init__(self, program=None, loc_origin=None, **kwargs):
        ...
        self.program = mglw.resources.programs.load(
            mglw.meta.ProgramDescription(
                ...
                path=Path("/HOME/data/panic3d/gameday-3d-human-reconstruction-panic3d_renderer-/_cells/shaders/rutileE.glsl").resolve(),
            )
        )
...
def main():
    ...
    for i,out in enumerate(outs):
        ...
        out.resize(mini_size, mini_mode).save(mkfile(f'{odn}/{render_dtype}/{franch}/{bn}/{i:04d}.png'))
